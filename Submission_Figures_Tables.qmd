...
conc_stats <- conc_data |>
  group_by(year) |>
  summarise(
    median_no3 = median(nitrate_mg_l, na.rm = TRUE),
    q25 = quantile(nitrate_mg_l, 0.25, na.rm = TRUE),
    q75 = quantile(nitrate_mg_l, 0.75, na.rm = TRUE),
    max_no3 = max(nitrate_mg_l, na.rm = TRUE),
    .groups = "drop"
  )

# Panel A: Nitrate Concentration Density
p_density <- ggplot(conc_data, aes(x = nitrate_mg_l, fill = as.factor(year))) +
  geom_density(alpha = 0.5, color = "black", linewidth = 0.3) +
  geom_vline(data = conc_stats, aes(xintercept = median_no3, color = as.factor(year)),
             linetype = "dashed", linewidth = 0.8) +
  scale_fill_manual(values = c("2023" = "#E41A1C", "2024" = "#377EB8")) +
  ...
  annotate("text", x = Inf, y = Inf, hjust = 1.1, vjust = 1.5, size = 3,
           label = paste0(
             "2023: Median = ", round(conc_stats$median_no3[1], 1), 
             ", IQR = ", round(conc_stats$q25[1], 1), "-", round(conc_stats$q75[1], 1), "\n",
             "2024: Median = ", round(conc_stats$median_no3[2], 1),
             ", IQR = ", round(conc_stats$q25[2], 1), "-", round(conc_stats$q75[2], 1)
           )) +
  theme_publication(base_size = 10) +
  theme(legend.position = c(0.85, 0.7))

# Panel B: Proportion NO3 by Treatment (boxplot + jitter)
prop_data <- conc_data |>
  filter(treatment %in% c("Sorghum", "Sorghum + Rye", "Corn", "Soy")) |>
  mutate(Treatment = if_else(treatment == "Sorghum + Rye", "Sorghum + WCC", treatment))

p_prop <- ggplot(prop_data, aes(x = Treatment, y = prop_nitrate, fill = Treatment)) +
  geom_boxplot(alpha = 0.7, outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.15, size = 0.8) +
  geom_hline(yintercept = 0.9, linetype = "dotted", color = "grey40") +
  scale_fill_manual(values = treatment_colors) +
...
# Save
ggsave("figures/figureX_nitrate_concentration_proportion.png", fig_n_speciation,
