...
gdd_plot_v2 <- ggplot() +
  ...
  scale_x_date(date_breaks = "2 months", date_labels = "%b") +
  labs(x = "Month", y = "Cumulative Growing Degree Days (GDD)") +
  theme_publication(base_size = 10)
...
library(gt)
str(flux_data)
# Table S1: Use RAW flux values (including negatives) for min; round to whole numbers
# If 'gnha_day' column exists use it for min; otherwise fall back to gnha_day_no_negative
raw_flux_col <- if ("gnha_day" %in% names(flux_data)) "gnha_day" else "gnha_day_no_negative"

daily_flux_stats <- flux_data |>
  filter(treatment %in% c("Sorghum", "Sorghum + Rye", "Corn", "Soy")) |>
  mutate(
    Treatment = if_else(treatment == "Sorghum + Rye", "Sorghum + WCC", treatment),
    year = year(date),
    raw_flux = .data[[raw_flux_col]]
  ) |>
  filter(year %in% c(2023, 2024)) |>

  # --- STEP 1: POOL ROW & INTERROW ---
  group_by(date, plot, Treatment, year) |>
  summarise(
    plot_daily_flux = mean(gnha_day_no_negative, na.rm = TRUE),
    plot_daily_flux_raw = mean(raw_flux, na.rm = TRUE),
    .groups = "drop"
  ) |>

  # --- STEP 2: CALCULATE STATS — use raw for min to capture negatives ---
  group_by(Treatment, year) |>
  summarise(
    mean_flux = mean(plot_daily_flux, na.rm = TRUE),
    median_flux = median(plot_daily_flux, na.rm = TRUE),
    min_flux = min(plot_daily_flux_raw, na.rm = TRUE),   # negative values preserved
    max_flux = max(plot_daily_flux, na.rm = TRUE),
    .groups = "drop"
  )
daily_flux_stats |>
  group_by(Treatment) |>
  summarise(mean = mean(mean_flux))

print(daily_flux_stats)
# Create formatted table — round to nearest whole number per McDaniel
supp_table1 <- daily_flux_stats %>%
  gt() %>%
  cols_label(
    Treatment = "Treatment",
    year = "Year",
  ...
  fmt_number(
    columns = c(mean_flux, median_flux, min_flux, max_flux),
    decimals = 0  # Round to nearest whole number
  ) %>%
  tab_header(
    title = md("**Table S1. Daily N₂O Flux Statistics**"),
    subtitle = "Values represent plot-level averages (pooled row/interrow) in g N₂O-N ha⁻¹ d⁻¹. Min values include negative (uptake) fluxes."
  ) %>%
...
# 4. PLOTS: Split by year for red/blue title bars, swap Row/Interrow order
# Row first (control), Interrow second
row_interrow_comparison <- row_interrow_comparison |>
  mutate(rowvs_interrow = factor(rowvs_interrow, levels = c("Row", "Interrow")))

diff_analysis <- diff_analysis |>
  mutate(year = year(date))

# --- Panel (a): Absolute fluxes, 2023 ---
p_s5a <- ggplot(filter(row_interrow_comparison, year == 2023),
                aes(x = gnha_day_no_negative, color = rowvs_interrow)) +
  geom_boxplot(aes(y = Treatment), outlier.shape = NA, alpha = 0.7) +
  geom_jitter(aes(y = Treatment), position = position_jitterdodge(jitter.width = 0.2),
              alpha = 0.5, size = 0.7) +
  scale_x_log10() +
  annotation_logticks(sides = "b") +
  scale_color_manual(values = c("Row" = "grey80", "Interrow" = "grey40"),
                     name = "Position") +
  labs(y = "", x = expression("N"[2]*"O Flux (g N ha"^-1*" day"^-1*")"),
       tag = "(a)", title = "2023") +
  theme_publication(base_size = 10) +
  theme(legend.position = "top",
...
# --- Panel (b): Absolute fluxes, 2024 ---
p_s5b <- ggplot(filter(row_interrow_comparison, year == 2024),
                aes(x = gnha_day_no_negative, color = rowvs_interrow)) +
  geom_boxplot(aes(y = Treatment), outlier.shape = NA, alpha = 0.7) +
  geom_jitter(aes(y = Treatment), position = position_jitterdodge(jitter.width = 0.2),
              alpha = 0.5, size = 0.7) +
...
# --- Panel (c): Difference histogram, 2023 ---
p_s5c <- ggplot(filter(diff_analysis, year == 2023),
                aes(x = Diff, fill = Treatment)) +
  ...
  scale_fill_manual(values = treatment_colors) +
  labs(x = expression("Difference (g N ha"^-1*" day"^-1*")"),
       y = "Count", tag = "(c)", title = "2023") +
  theme_publication(base_size = 10) +
  theme(legend.position = "none",
        plot.tag = element_text(face = "bold", size = 11),
        plot.tag.position = "topleft",
...
# --- Panel (d): Difference histogram, 2024 ---
p_s5d <- ggplot(filter(diff_analysis, year == 2024),
                aes(x = Diff, fill = Treatment)) +
  geom_histogram(bins = 30, alpha = 0.7, position = "identity", color = "black",
                 linewidth = 0.3) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "red", linewidth = 0.6) +
...
# Display & assemble
rowvsinterrow <- (p_s5a | p_s5b) / (p_s5c | p_s5d)
rowvsinterrow
...
leaching_data |>
  ...
  slice_max(max_nitrate, by = c(treatment, year))
...
conc_data <- leaching_data |>
  ...
  filter(total_inorganic_n > 0) # Avoid division issues

# All treatments with renamed labels and order
conc_all <- conc_data |>
  mutate(
    Treatment = case_when(
...
# Also keep Sorghum vs WCC subset for t-test
conc_sr <- conc_all |>
  filter(Treatment %in% c("Sorghum", "Sorghum + WCC"))

# Medians for vertical lines
conc_medians <- conc_all |>
  group_by(Treatment) |>
  summarise(median_no3 = median(nitrate_mg_l, na.rm = TRUE), .groups = "drop")

# t-test on mean concentrations (Sorghum vs WCC)
ttest <- t.test(nitrate_mg_l ~ Treatment, data = conc_sr)
ttest_table <- tibble(
  comparison = "Sorghum vs Sorghum + WCC",
  mean_sorghum = mean(conc_sr$nitrate_mg_l[conc_sr$Treatment == "Sorghum"], na.rm = TRUE),
  mean_wcc = mean(conc_sr$nitrate_mg_l[conc_sr$Treatment == "Sorghum + WCC"], na.rm = TRUE),
  diff_means = mean_sorghum - mean_wcc,
  t_stat = unname(ttest$statistic),
  df = unname(ttest$parameter),
  p_value = ttest$p.value
)

# Subtle colors for Maize/Soybean
s4_colors <- c("Maize"          = "#B0B0B0",
               "Soybean"        = "#A8C8E8",
               "Sorghum"        = "#D8D97AFF",
               "Sorghum + WCC"  = "#95C36EFF")

# Panel (a): Density plot — all treatments
p_s4a <- ggplot(conc_all, aes(x = nitrate_mg_l, fill = Treatment, color = Treatment)) +
  geom_density(aes(y = after_stat(density * 100)), alpha = 0.5, linewidth = 0.3) +
  geom_vline(data = conc_medians, aes(xintercept = median_no3, color = Treatment),
  ...
  scale_fill_manual(values = s4_colors) +
  scale_color_manual(values = s4_colors) +
  theme_publication(base_size = 10) +
  theme(legend.position = "top",
        plot.tag = element_text(face = "bold", size = 11),
        plot.tag.position = "topleft")

# Panel (b): Proportion nitrate boxplots — all treatments, y trimmed to 50% min
p_s4b <- ggplot(conc_all, aes(x = Treatment, y = prop_nitrate * 100, fill = Treatment)) +
  ...
  labs(
    tag = "(b)",
    x = "Treatment",
    y = expression("NO"[3]^{"-"}*"-N as % of Total Inorganic N")
  ) +
  theme_publication(base_size = 10) +
  theme(legend.position = "none",
...
ttest_table |>
  mutate(
    mean_sorghum = round(mean_sorghum, 3),
    mean_wcc = round(mean_wcc, 3),
    diff_means = round(diff_means, 3),
  ...
  cols_label(
    ...
    mean_sorghum = "Mean NO3 (Sorghum)",
    mean_wcc = "Mean NO3 (Sorghum + WCC)",
    diff_means = "Mean Difference",
    t_stat = "t",
    df = "df",
    p_value = "p"
  )
...
# Save
ggsave("figures/FigureS4_nitrate_concentration_proportion.tiff", fig_s4,
       width = 174, height = 150, units = "mm", dpi = 600, bg = "white",
       compression = "lzw")

# Measurement counts by year and plot (concentration data only)
# Ensure year exists
if (!("year" %in% names(conc_data))) {
  conc_data <- conc_data |>
    mutate(date = as.Date(date), year = lubridate::year(date))
}

conc_counts_by_year_plot <- conc_data |>
  group_by(year, treatment, plot) |>
  summarise(n_measurements = n(), .groups = "drop") |>
  arrange(year, treatment, plot)

conc_counts_by_year_plot |>
  gt() |>
  cols_label(
    year = "Year",
    treatment = "Treatment",
...
# Summary counts by year and treatment
conc_counts_by_year_treatment <- conc_data |>
  group_by(year, treatment) |>
  summarise(n_measurements = n(), .groups = "drop") |>
  arrange(year, treatment)

conc_counts_by_year_treatment |>
  gt() |>
  cols_label(
    year = "Year",
    treatment = "Treatment",
...
# Figure S_NEW: Stacked Bar — NH4/NO3 Proportions Over Time

```{r figure-s-nh4-no3-stacked}
#| message: FALSE
#| warning: FALSE

# Prepare time-series of NH4 vs NO3 proportions by treatment and sampling date
nh4_no3_ts <- conc_data |>
  mutate(
    date = as.Date(date),
  ...
  group_by(Treatment, year, date) |>
  summarise(
    mean_no3 = mean(nitrate_mg_l, na.rm = TRUE),
    mean_nh4 = mean(ammonia_mg_l, na.rm = TRUE),
    .groups = "drop"
  ) |>
  mutate(total = mean_no3 + mean_nh4,
         pct_no3 = mean_no3 / total * 100,
         pct_nh4 = mean_nh4 / total * 100) |>
  pivot_longer(cols = c(pct_no3, pct_nh4),
               names_to = "N_form", values_to = "pct") |>
  mutate(
    N_form = if_else(N_form == "pct_no3",
                     "NO\u2083\u207B-N", "NH\u2084\u207A-N"),
    N_form = factor(N_form, levels = c("NH\u2084\u207A-N", "NO\u2083\u207B-N"))
  )

# Standardize dates to a common year for overlay
nh4_no3_ts <- nh4_no3_ts |>
  mutate(plot_date = as.Date(paste0("2000-", format(date, "%m-%d"))))

# Colors for N forms
n_form_colors <- c("NO\u2083\u207B-N" = "#4393C3",   # blue
                    "NH\u2084\u207A-N" = "#D6604D")   # red-orange

# Stacked bar per year
make_nh4no3_panel <- function(yr, tag_label, year_color) {
  df <- filter(nh4_no3_ts, year == yr)
  ggplot(df, aes(x = plot_date, y = pct, fill = N_form)) +
    geom_col(position = "stack", width = 5, color = NA) +
    facet_wrap(~ Treatment, nrow = 1) +
    scale_x_date(date_breaks = "2 months", date_labels = "%b") +
    scale_fill_manual(values = n_form_colors, name = "N Form") +
    labs(x = "", y = "Proportion (%)", tag = tag_label,
...
p_nh4no3_a <- make_nh4no3_panel(2023, "(a)", "#E41A1C")
p_nh4no3_b <- make_nh4no3_panel(2024, "(b)", "#377EB8")

fig_nh4no3 <- p_nh4no3_a / p_nh4no3_b
fig_nh4no3

ggsave("figures/FigureS_NH4_NO3_proportions.tiff", fig_nh4no3,
       width = 174, height = 130, units = "mm", dpi = 600, bg = "white",
       compression = "lzw")
```
...
plot_hydrology <- annual_leaching %>%
  select(year, plot, treatment, drainage_mm = annual_drainage_mm,
         leaching_kg_ha = annual_cumulative_leaching) %>%
  left_join(texture, by = c("plot" = "Plot")) %>%
  mutate(
    year = as.factor(year),
    # Rename Corn → Maize, Soy → Soybean for publication
    Treatment = case_when(
      treatment == "Corn"          ~ "Maize",
      treatment == "Soy"           ~ "Soybean",
      treatment == "Sorghum + Rye" ~ "Sorghum + WCC",
...
# Subtle colors for Maize/Soybean, standard for Sorghum treatments
supp_colors <- c("Maize"          = "#B0B0B0",   # gray
                  "Soybean"        = "#A8C8E8",   # light blue
                  "Sorghum"        = "#D8D97AFF",
                  "Sorghum + WCC"  = "#95C36EFF")
...
# Helper: extract per-treatment lm equation + R² annotation labels
  ...
  df |>
    ...
    summarise(
      lbl = {
        ...
        b0 <- round(coef(mod)[1], 1)
        b1 <- round(coef(mod)[2], 2)
        r2 <- round(summary(mod)$r.squared, 2)
        paste0("y = ", b1, "x + ", b0, "  R\u00b2 = ", r2)
      },
      x_pos = max(get(xvar), na.rm = TRUE),
      y_pos = max(get(yvar), na.rm = TRUE),
      .groups = "drop"
    )
}
...
# PANEL (a): Drainage vs. N Leaching Load (swapped from old Panel B)
...
p_loss <- ggplot(plot_hydrology, aes(x = drainage_mm, y = leaching_kg_ha)) +
  # Per-treatment regression lines
  geom_smooth(aes(color = Treatment), method = "lm", se = FALSE, linewidth = 0.7) +
  # Points with shape differences
  ...
  # Annotate per-treatment equations
  geom_text(data = eq_loss, aes(x = -Inf, y = Inf, label = lbl, color = Treatment),
            hjust = -0.05, vjust = seq(1.5, by = 1.3, length.out = nrow(eq_loss)),
            size = 2.8, show.legend = FALSE, fontface = "italic") +
  labs(tag = "(a)", x = "Annual Drainage (mm)",
...
# PANEL (b): Texture vs. Drainage (swapped from old Panel A)
...
p_texture <- ggplot(plot_hydrology, aes(x = Sand, y = drainage_mm)) +
  ...
  scale_color_manual(values = supp_colors) +
  scale_shape_manual(values = supp_shapes) +
  geom_text(data = eq_tex, aes(x = -Inf, y = Inf, label = lbl, color = Treatment),
            hjust = -0.05, vjust = seq(1.5, by = 1.3, length.out = nrow(eq_tex)),
            size = 2.8, show.legend = FALSE, fontface = "italic") +
  labs(tag = "(b)", x = "Sand Content (%)",
...
# --- 4. COMBINE & SAVE ---
figure_s3 <- p_loss + p_texture + plot_layout(widths = c(1, 1))

print(figure_s3)
ggsave("FigureS_Hydrology.tiff", figure_s3, width = 174, height = 90,
...
p_hydro <- ggplot(hydrograph_data, aes(x = date, y = cumulative_mm_reset, group = plot, color = treatment)) +
  ...
  # Colors — subtle Maize/Soybean
  scale_color_manual(values = c("Maize"          = "#B0B0B0",
                                "Soybean"        = "#A8C8E8",
                                "Sorghum"        = "#D8D97AFF",
                                "Sorghum + WCC"  = "#95C36EFF")) +

  # Labels — no title per McDaniel
  labs(x = "Date",
...
plot_hydrology$treatment <- relevel(factor(plot_hydrology$treatment), ref = "Sorghum + Rye")
...
# Table S2: Cover Crop Biomass

```{r table-s2-cover-crop-biomass}
#| message: FALSE
#| warning: FALSE
...
# Load rye cover crop biomass data
...
table_s2 <- rye_biomass_summary |>
  ...
  tab_header(
    ...
    subtitle = "Biomass in Mg ha⁻¹, measured at termination"
  ) |>
  cols_align(align = "center", columns = everything())
...
Save to SubmissionFiguresTables

```{r}
#| label: save-submission-materials
...
# Figure 1: Weather
ggsave(
  "SubmissionFiguresTables/Figure1_Weather.png",
  plot = six_panel_plot,
  width = 84, height = 105, units = "mm", dpi = 600, bg = "white"
)

# Figure 2: Soil Moisture & Temperature
ggsave(
  "SubmissionFiguresTables/Figure2_Soil_Moisture_Temperature.png",
  plot = soil_4panel,
  width = 174, height = 174, units = "mm", dpi = 600, bg = "white"
)

# Figure 3: N2O Flux & Precipitation (Dual Axis)
ggsave(
  "SubmissionFiguresTables/Figure3_N2O_Flux_Precipitation.png",
  plot = figure3_dual_axis,
  width = 174, height = 174, units = "mm", dpi = 600, bg = "white"
)

# Figure 4: Leaching & Precipitation
ggsave(
  "SubmissionFiguresTables/Figure4_Leaching_Precipitation.png",
  plot = fig_leaching,
  width = 174, height = 174, units = "mm", dpi = 600, bg = "white"
)

# Figure 5: Nitrate Speciation
ggsave(
...
# Figure 6: Hydrology
ggsave(
  "SubmissionFiguresTables/Figure6_Hydrology.png",
  plot = p_hydro,
  width = 174, height = 87, units = "mm", dpi = 600, bg = "white"
)

# Supp Figure row vs interrow
ggsave(
  "SubmissionFiguresTables/rowvsinterrow.png",
  plot = rowvsinterrow,
    width = 174, height = 200, units = "mm", dpi = 600, bg = "white"
)

# --- TABLES ---
# Table 1: Daily Flux Statistics (Supplementary)
gtsave(
  supp_table1,
  "SubmissionFiguresTables/Table_S1_Daily_Flux_Statistics.rtf"
)

# Table 2: Validation (Cumulative N2O)
gtsave(
  validation_table,
  "SubmissionFiguresTables/Table_S2_Validation_Cumulative_N2O.png"
)

# Table 3: Annual Nitrogen Losses & Environmental Costs
gtsave(
  ft,
  "SubmissionFiguresTables/Table3_Annual_Nitrogen_Losses.rtf"
)

cat("\n✓ All figures and tables saved to SubmissionFiguresTables/\n")
```