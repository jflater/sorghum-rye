...
```{r}
library(dplyr)
# Note: 'gt' package not available, using knitr::kable instead
...
validation_table <- validation_stats |>
  mutate(
    ...
    CI_Lower = round(cum_flux_mean - cum_flux_ci, 2),
    CI_Upper = round(cum_flux_mean + cum_flux_ci, 2),
...
# 3. Display and Save
cat("### Validation of Cumulative N₂O Statement\n")
cat("Verifying: 'Reduced non-significantly (Range: 0.9 to 1.6 kg N)'\n\n")
knitr::kable(validation_table, 
             col.names = c("Year", "Treatment", "Mean Cumulative Flux (SE)", 
                          "95% CI Lower", "95% CI Upper", "Text Validation"),
             caption = "Validation of Cumulative N₂O Statement",
             align = c('l', 'l', 'r', 'r', 'r', 'l'))

# Save as CSV since PNG generation requires gt package
write.csv(validation_table, "figures/validation_table_cumulative_n2o.csv", row.names = FALSE)
cat("\nTable saved to: figures/validation_table_cumulative_n2o.csv\n")
```
...
# --- 5. Generate Table Output ---
# Note: 'gt' package not available, using knitr::kable instead

table3_display <- table3_complete |>
  select(
    Year = year_display,
    Treatment = treatment,
    `N2O Loss (kg N/ha)` = n2o_result,
    `N Leaching (kg N/ha)` = leaching_result, 
    `Total Environmental Cost (USD/ha)` = total_environmental_cost
  )

cat("### Table 3. Annual nitrogen losses\n\n")
knitr::kable(table3_display, 
             caption = "Table 3. Annual nitrogen losses",
             align = c('l', 'l', 'r', 'r', 'r'))

cat("\n\n**Note:** Values are mean ± standard deviation. Letters (in parentheses) indicate Tukey-adjusted groups within year (α = 0.05). Two-year totals (2023–2024) show sum of annual means. Environmental cost calculated using Preza-Fontes values: $16.18/kg N₂O and $18.54/kg N leaching.\n")

# Save table as CSV
write.csv(table3_display, "tables/table3_nitrogen_losses.csv", row.names = FALSE)
cat("\nTable saved to: tables/table3_nitrogen_losses.csv\n")
```
