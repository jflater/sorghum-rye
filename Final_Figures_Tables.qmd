...
check_normality <- function(data, variable, group_vars) {
  data %>%
    group_by(across(all_of(group_vars))) %>%
    group_modify(~ {
      df <- .x %>% drop_na({{ variable }})
      # Shapiro per treatment
      by_trt <- df %>%
        group_by(treatment) %>%
        shapiro_test(as.formula(paste(variable, "~ 1"))) %>%
        transmute(scope = "by_treatment", treatment, p = p, method = "Shapiro-Wilk")
      # Shapiro on residuals from pooled model (tighter to testing assumption)
      resid_test <- df %>%
        lm(as.formula(paste(variable, "~ treatment"))) %>%
        rstatix::augment() %>%
        shapiro_test(.resid) %>%
        transmute(scope = "residuals", treatment = NA_character_, p = p, method = "Shapiro-Wilk")
      bind_rows(by_trt, resid_test)
    }) %>%
    ungroup()
}

compare_soil_inorgN <- function(data, variable, group_vars) {
  data %>%
    group_by(across(all_of(group_vars))) %>%
    group_modify(~ {
      df <- .x %>% drop_na({{ variable }})
      trt <- sort(unique(df$treatment))
      if (length(trt) < 2 || any(table(df$treatment) < 2)) {
        return(tibble(test = "omnibus", trt1 = NA_character_, trt2 = NA_character_,
                      p = NA_real_, p_raw = NA_real_, method = "insufficient reps"))
      }

      if (length(trt) == 2) {
        pw <- df %>%
          t_test(as.formula(paste(variable, "~ treatment")), var.equal = FALSE) %>%
          adjust_pvalue(method = "holm") %>%
          add_significance() %>%
          transmute(test = "pairwise", trt1 = group1, trt2 = group2,
                    p = p.adj, p_raw = p, method = "Welch t (Holm)")
        return(pw)
      } else {
        omni <- df %>%
          welch_anova_test(as.formula(paste(variable, "~ treatment"))) %>%
          transmute(test = "omnibus", trt1 = NA_character_, trt2 = NA_character_,
                    p = p, p_raw = p, method = "Welch ANOVA")

        pw <- df %>%
          pairwise_t_test(as.formula(paste(variable, "~ treatment")),
                          p.adjust.method = "holm", pool.sd = FALSE) %>%
          add_significance() %>%
          transmute(test = "pairwise", trt1 = group1, trt2 = group2,
                    p = p.adj, p_raw = p, method = "Welch t (Holm)")

        return(bind_rows(omni, pw))
      }
    }) %>%
    ungroup()
}
...
check_normality(soil_data, "nitrate_ppm", c("year", "date"))


compare_soil_inorgN(soil_data, "nitrate_ppm", c("year", "date"))
...
soil_4panel
ggsave("figures/figure2_soil_moisture_temperature.png", soil_4panel, width = 174, height = 174, dpi = 600, bg = "white", units = "mm")
...
ggsave("figures/figure1_weather.png", six_panel_plot, width = 174, height = 174, dpi = 600, bg = "white", units = "mm")
six_panel_plot
