...
# Get leaching measurement date ranges for each year (similar to Figure 2)
leaching_date_ranges <- leaching_data %>%
  filter(treatment %in% c("Sorghum", "Sorghum + Rye")) %>%
  group_by(year) %>%
  summarise(
    min_date = floor_date(min(date, na.rm = TRUE), "month"),  # Align with monthly data
    max_date = ceiling_date(max(date, na.rm = TRUE), "month") - days(1),  # End of last month with data
    .groups = "drop"
  )

print(leaching_date_ranges)
cat("Leaching measurement ranges:\n")
print(leaching_date_ranges)

# Build cumulative leaching data using the same approach as Figure 2
# First get daily leaching rates per plot
...
# Build cumulative leaching using same interpolation approach as Figure 2
cum_leaching_combined <- daily_leaching_summary %>%
  ...
  select(-start_date, -end_date) %>%
  # Join with observed data
  left_join(daily_leaching_summary, by = c("treatment", "year", "date")) %>%
  group_by(treatment, year) %>%
  # Interpolate within observed ranges only (rule = 1)
  mutate(
    mean_loss_interp = approx(
      x = date[!is.na(mean_loss)],
      y = mean_loss[!is.na(mean_loss)],
      xout = date, rule = 1, ties = "ordered"
    )$y,
    se_loss_interp = approx(
      x = date[!is.na(se_loss)],
      y = se_loss[!is.na(se_loss)],
      xout = date, rule = 1, ties = "ordered"
    )$y
  ) %>%
  # Filter to only include dates within measurement range
  filter(!is.na(mean_loss_interp)) %>%
  # Calculate cumulative
  mutate(
    cum_loss = cumsum(mean_loss_interp),
    cum_se = sqrt(cumsum(se_loss_interp^2))
  ) %>%
  ungroup()

# Create base theme for consistent formatting
base_theme_leaching <- theme_publication() +
  theme(
...
# 1) Monthly leaching loss (faceted, top row) - using daily summary data aggregated by month
monthly_leaching <- daily_leaching_summary %>%
  # Trim to measurement date ranges
  left_join(leaching_date_ranges, by = "year") %>%
  filter(date >= min_date, date <= max_date) %>%
...
monthly_plot <- ggplot(monthly_leaching, aes(x = month_year, y = monthly_total, fill = treatment)) +
  ...
  # Add invisible points to set x-axis limits based on measurement ranges
  geom_blank(data = leaching_date_ranges, inherit.aes = FALSE,
             aes(x = min_date, y = 0)) +
  geom_blank(data = leaching_date_ranges, inherit.aes = FALSE,
             aes(x = max_date, y = 0)) +
  scale_fill_treatments() +
...
monthly_plot
# 2) Cumulative leaching plot (faceted, bottom row with x-axis labels)
...
cumulative_leaching_plot
# Assemble final figure: 2 faceted rows
