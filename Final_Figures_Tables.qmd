...
daily_flux_plot <- ggplot(daily_flux, aes(x = date, y = mean_flux, color = Treatment)) +
  ...
  theme(
    axis.text.x = element_blank(),
    axis.ticks.x = element_blank(),
    strip.text = element_blank()  # Hide strip text for middle row
  )
...
# Prepare daily leaching data for Sorghum systems
leaching_daily <- leaching_data %>%
  filter(treatment %in% c("Sorghum", "Sorghum + Rye")) %>%
  mutate(
    total_n_loss_kgha = (total_n_loss_mg / 1e6) / plot_area_ha,
    cumulative_n_loss_kgha = (cumulative_n_loss_mg / 1e6) / plot_area_ha
  ) %>%
  group_by(treatment, year, date) %>%
  summarise(
    mean_loss = mean(total_n_loss_kgha, na.rm = TRUE),
    se_loss = sd(total_n_loss_kgha, na.rm = TRUE) / sqrt(sum(!is.na(total_n_loss_kgha))),
    mean_cum = mean(cumulative_n_loss_kgha, na.rm = TRUE),
    se_cum = sd(cumulative_n_loss_kgha, na.rm = TRUE) / sqrt(sum(!is.na(cumulative_n_loss_kgha))),
    .groups = "drop"
  )

# Axis limits
max_daily <- max(leaching_daily$mean_loss + leaching_daily$se_loss, na.rm = TRUE)
max_cum <- max(leaching_daily$mean_cum + leaching_daily$se_cum, na.rm = TRUE)

# Top panels: event total N loss
loss_2023 <- leaching_daily %>% filter(year == 2023)
loss_2024 <- leaching_daily %>% filter(year == 2024)

top_2023 <- ggplot(loss_2023, aes(x = date, y = mean_loss, fill = treatment)) +
  geom_col(position = position_dodge(width = 0.9), width = 0.8) +
  geom_errorbar(aes(ymin = pmax(mean_loss - se_loss, 0), ymax = mean_loss + se_loss),
                position = position_dodge(width = 0.9), width = 0.2) +
  scale_fill_treatments() +
  scale_y_continuous(limits = c(0, max_daily), expand = expansion(mult = c(0, 0.05))) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b",
               limits = c(as.Date("2023-01-01"), as.Date("2023-12-31"))) +
  labs(x = NULL, y = expression("Event N loss (kg N ha"^{-1}*")"),
       fill = "Treatment", title = "2023") +
  theme_publication() +
  theme(legend.position = "none", axis.text.x = element_blank(), axis.ticks.x = element_blank())

top_2024 <- ggplot(loss_2024, aes(x = date, y = mean_loss, fill = treatment)) +
  geom_col(position = position_dodge(width = 0.9), width = 0.8) +
  geom_errorbar(aes(ymin = pmax(mean_loss - se_loss, 0), ymax = mean_loss + se_loss),
                position = position_dodge(width = 0.9), width = 0.2) +
  scale_fill_treatments() +
  ...
  scale_x_date(date_breaks = "1 month", date_labels = "%b",
               limits = c(as.Date("2024-01-01"), as.Date("2024-12-31"))) +
  labs(x = NULL, y = NULL, fill = "Treatment", title = "2024") +
  theme_publication() +
  theme(legend.position = "none", axis.text.x = element_blank(), axis.ticks.x = element_blank())

# Bottom panels: cumulative N loss
cum_2023 <- ggplot(loss_2023, aes(x = date, y = mean_cum, color = treatment, fill = treatment)) +
  geom_ribbon(aes(ymin = pmax(mean_cum - se_cum, 0), ymax = mean_cum + se_cum),
              alpha = 0.2, linewidth = 0) +
  geom_line(linewidth = 1.1) +
  scale_color_treatments() +
  scale_fill_treatments() +
  scale_y_continuous(limits = c(0, max_cum), expand = expansion(mult = c(0, 0.05))) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b",
               limits = c(as.Date("2023-01-01"), as.Date("2023-12-31"))) +
  labs(x = "Month", y = expression("Cumulative N loss (kg N ha"^{-1}*")"),
       color = "Treatment", fill = "Treatment") +
  theme_publication() +
  theme(legend.position = "none")

cum_2024 <- ggplot(loss_2024, aes(x = date, y = mean_cum, color = treatment, fill = treatment)) +
  geom_ribbon(aes(ymin = pmax(mean_cum - se_cum, 0), ymax = mean_cum + se_cum),
              alpha = 0.2, linewidth = 0) +
  geom_line(linewidth = 1.1) +
  scale_color_treatments() +
  ...
  scale_x_date(date_breaks = "1 month", date_labels = "%b",
               limits = c(as.Date("2024-01-01"), as.Date("2024-12-31"))) +
  labs(x = "Month", y = NULL, color = "Treatment", fill = "Treatment") +
  theme_publication() +
  theme(legend.position = "bottom")

# Combine panels
figure3 <- (top_2023 + top_2024) / (cum_2023 + cum_2024) +
...
#| fig-cap: "Stacked annual nitrogen losses by treatment"
#| fig-width: 10
#| fig-height: 6
...
# Significance letters for each component
get_letters <- function(df, value_col) {
  pw <- pairwise.t.test(df[[value_col]], df$treatment, p.adjust.method = "none")
  multcompView::multcompLetters(pw$p.value)$Letters %>%
    enframe(name = "treatment", value = "letter")
}
