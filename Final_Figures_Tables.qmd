...
create_dual_axis_plot <- function(year_data, year_val) {
  ...
  max_flux <- max(flux_year$mean_flux + flux_year$se_flux, na.rm = TRUE)
  max_precip <- max(precip_year$precipmm, na.rm = TRUE)

  # Scale precipitation to fit on flux scale (inverted from top)
  precip_scale_factor <- max_flux * 0.6 / max_precip  # Use 60% of flux range for precip

  # Create the plot
  p <- ggplot() +
    # Precipitation bars (inverted from top)
    geom_col(data = precip_year,
             aes(x = date, y = -precipmm * precip_scale_factor),
             fill = "lightblue", alpha = 0.7, width = 1) +
    ...
    # Primary y-axis (flux)
    scale_y_continuous(
      name = expression("Daily N"[2]*"O Flux (g N ha"^-1*" day"^-1*")"),
      sec.axis = sec_axis(~ -. / precip_scale_factor,
...
plot_2023 <- create_dual_axis_plot(daily_flux, 2023)
plot_2024 <- create_dual_axis_plot(daily_flux, 2024)
...
ggsave("figures/figure2_dual_axis_flux_precip_2023_2024.png",
       figure2_dual_axis, width = 12, height = 6, dpi = 300, bg = "white")
```

Figure 2 Alternative. Daily N₂O flux and precipitation on dual axes by year.
Precipitation bars "rain down" from the top (light blue, right y-axis) while daily N₂O flux is plotted along the bottom axis (colored lines, left y-axis). Asterisks (*) indicate dates where flux differs significantly between treatments (p < 0.05). Red dashed lines show fertilizer application dates.
...
create_leaching_dual_axis_plot <- function(year_val) {
  # Filter data for the specific year
  ...
  # Get date range for this year
  date_range <- leaching_date_ranges %>% filter(year == year_val)

  # Calculate scaling factors for dual axis
  max_loss <- max(leaching_year$mean_loss + leaching_year$se_loss, na.rm = TRUE)
  max_precip <- max(precip_year$precipmm, na.rm = TRUE)

  # Scale precipitation to fit on leaching scale (inverted from top)
  precip_scale_factor <- max_loss * 0.6 / max_precip  # Use 60% of leaching range for precip
  ...
  p <- ggplot() +
    ...
    # Primary y-axis (leaching)
    scale_y_continuous(
      name = expression("Daily N Loss (kg N ha"^-1*" day"^-1*")"),
      sec.axis = sec_axis(~ -. / precip_scale_factor,
                          name = "Daily Precipitation (mm)")
    ) +
...
daily_plot_2023 <- create_leaching_dual_axis_plot(2023)
daily_plot_2024 <- create_leaching_dual_axis_plot(2024)
...
# Modify cumulative plot to match Figure 2 structure (no strip text, bottom legend)
