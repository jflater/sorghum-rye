...
get_letters <- function(df, value_col) {
  ...
  trts <- sort(unique(df$treatment))
  # If fewer than 2 treatments present, assign 'a'
  if (length(trts) < 2) {
    return(tibble(treatment = trts, letter = "a"))
  }
  pw <- pairwise.t.test(df[[value_col]], df$treatment, p.adjust.method = "none", pool.sd = FALSE)
  # Build a full symmetric p-value matrix for all treatments; set missing/NA to 1 (not significant)
  full <- matrix(1, nrow = length(trts), ncol = length(trts), dimnames = list(trts, trts))
  if (!is.null(pw$p.value)) {
    full[rownames(pw$p.value), colnames(pw$p.value)] <- pw$p.value
  }
  full[lower.tri(full)] <- t(full)[lower.tri(full)]
  diag(full) <- 0
  full[is.na(full)] <- 1
  letters <- multcompView::multcompLetters(full)$Letters
  tibble(treatment = names(letters), letter = as.character(letters)) %>%
    tidyr::complete(treatment = trts, fill = list(letter = "a"))
}
