...
n2o_table_data <- plot_cumulative_n2o %>%
  ...
  summarise(
    n_plots = n(),
    mean_n2o = mean(final_cumulative_plot, na.rm = TRUE) / 1000,  # Convert g to kg
    sd_n2o = sd(final_cumulative_plot, na.rm = TRUE) / 1000,      # Convert g to kg
    .groups = "drop"
  ) %>%
  mutate(
    result = sprintf("%.3f ± %.3f", mean_n2o, sd_n2o)  # More decimals needed for kg values
  )
...
two_year_totals <- plot_cumulative_n2o %>%
  ...
  summarise(
    n_plots = n(),
    mean_n2o = mean(total_2yr_n2o, na.rm = TRUE) / 1000,  # Convert g to kg
    sd_n2o = sd(total_2yr_n2o, na.rm = TRUE) / 1000,      # Convert g to kg
    result = sprintf("%.3f ± %.3f", mean_n2o, sd_n2o),    # More decimals for kg
    .groups = "drop"
  ) %>%
  mutate(
    year = "2023-2024",
...
n_leaching_summary <- annual_n_leaching %>%
  ...
  summarise(
    n_plots = n(),
    mean_leaching = mean(max_cumulative_n, na.rm = TRUE) / 1000000,  # Convert mg to kg
    sd_leaching = sd(max_cumulative_n, na.rm = TRUE) / 1000000,      # Convert mg to kg
    .groups = "drop"
  )
...
n_leaching_table_data <- n_leaching_summary %>%
  mutate(
    result = sprintf("%.3f ± %.3f", mean_leaching, sd_leaching)  # 3 decimals for kg values
  ) %>%
...
two_year_leaching <- annual_n_leaching %>%
  ...
  group_by(treatment) %>%
  summarise(
    mean_leaching = mean(total_2yr_leaching, na.rm = TRUE) / 1000000,  # Convert mg to kg
    sd_leaching = sd(total_2yr_leaching, na.rm = TRUE) / 1000000,      # Convert mg to kg
    result = sprintf("%.3f ± %.3f", mean_leaching, sd_leaching),       # 3 decimals for kg
    .groups = "drop"
  ) %>%
...
table3_updated_gt <- table3_updated %>%
  ...
  cols_label(
    ...
    `N Leaching` = md("N Leaching<br>(kg N)"),
