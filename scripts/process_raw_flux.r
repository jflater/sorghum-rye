...
# Read in all .csv files, skipping the first row
# and merging the second and third rows as the header
custom_read_csv <- function(file) {
  # Read first three rows
  header_rows <- readr::read_csv(file,
                                 n_max = 3,
                                 col_names = FALSE,
...
# Here is the processed data, before proceding
# check if any dates already exist in processed data
...
file.rename(from = file_list,
            to = file.path("sorghum-rye/data/archive", basename(file_list)))
