...
compute_tukey_letters <- function(data, variable, group_vars) {
  # This function computes Tukey HSD letters for a given variable

  # within each combination of grouping variables (e.g., year, date)
  ...
  data %>%
    ...
    group_modify(~ {
      ...
      # Fit linear model
      fml <- as.formula(paste(variable, "~ treatment"))
      fit <- tryCatch(
        lm(fml, data = df),
      ...
      cld_result <- tryCatch(
        multcomp::cld(emm, adjust = "tukey", Letters = letters, alpha = 0.05) %>%
          ...
          mutate(
            # If numbers slipped through, map them to letters
...
extract_pvals <- function(aov_model) {
  ...
  # Get row names to find correct indices
  row_names <- rownames(summary_aov)
  
  # Find indices for date, treatment, and interaction
  date_idx <- grep("date_factor", row_names)[1]
  trt_idx <- grep("^treatment$", row_names)
  int_idx <- grep(":", row_names)[1]
  
  pvals <- c(
    Date = summary_aov[date_idx, "Pr(>F)"],
    Treatment = summary_aov[trt_idx, "Pr(>F)"],
    `Date × Treatment` = summary_aov[int_idx, "Pr(>F)"]
  )
