...
This document reproduces Figure 2 (daily N₂O flux and cumulative flux) using a more
statistically appropriate uncertainty method: plot-level cumulative curves with
...
# Plot-Level Cumulative Flux Computation

The key improvement here is computing cumulative flux at the **plot level** first,
...
# Step 1: Summarize daily flux data at the plot level
# Group by plot, rowvs_interrow, treatment, year, date to get mean flux per plot per date
daily_flux_plot <- flux_data %>%
  filter(treatment %in% c("Sorghum", "Sorghum + Rye")) %>%
  mutate(year = lubridate::year(date)) %>%
  filter(year %in% analysis_years) %>%
  group_by(plot, treatment, year, date) %>%
  summarise(
    flux = mean(gnha_day_no_negative, na.rm = TRUE) / 1000,  # Convert g to kg N ha-1 day-1
    .groups = "drop"
  ) %>%
  arrange(treatment, year, plot, date)

# Step 2: Get measurement date ranges for each year
flux_date_ranges <- daily_flux_plot %>%
  group_by(year) %>%
  summarise(
...
cat("Flux measurement date ranges:\n")
print(flux_date_ranges)

# Step 3: Interpolate daily fluxes and compute cumulative flux at plot level
# Track plots with insufficient data
plots_excluded <- daily_flux_plot %>%
...
cum_flux_plot_level <- daily_flux_plot %>%
  ...
  group_modify(~ {
    ...
    yr <- unique(df$year)
    date_range <- flux_date_ranges %>% filter(year == yr)
    ...
    # Interpolate flux values (rule = 1 means no extrapolation beyond observed range)
    n_pts <- sum(!is.na(df$flux))
    if (n_pts >= 2) {
      flux_interp <- approx(
        x = df$date[!is.na(df$flux)],
        y = df$flux[!is.na(df$flux)],
        xout = df_full$date,
        rule = 1,
        ties = "ordered"
      )$y
    } else {
      flux_interp <- df_full$flux
    }
    
    df_full %>%
      mutate(
        flux_interp = flux_interp,
        has_sample = !is.na(flux)
      ) %>%
...
cat("\nPlot-level cumulative flux data:", nrow(cum_flux_plot_level), "observations\n")
cat("Unique plots:", length(unique(cum_flux_plot_level$plot)), "\n")
...
cum_flux_summary <- cum_flux_plot_level %>%
  ...
  mutate(
    cum_flux_ci = if_else(is.na(cum_flux_ci), 0, cum_flux_ci)
  )

cat("\nSummary statistics computed for", nrow(cum_flux_summary), "treatment-year-date combinations\n")

# Get points where actual samples were taken for plotting
cum_flux_points <- cum_flux_summary %>% filter(has_sample)
```

# Daily Flux Summary (for top panel)

```{r daily-flux-summary}
#| label: daily-flux-summary

# Build daily treatment means and SE for N2O flux (same as original Figure 2)
...
```{r figure2-dual-axis}
#| label: fig-flux-plotlevel-ci
#| fig-cap: "Daily and cumulative N2O flux with plot-level cumulative uncertainty (95% CI)"
#| fig-width: 13.3
#| fig-height: 7.5
...
create_dual_axis_plot <- function(year_val, is_right_panel = FALSE) {
  ...
  daily_plot <- ggplot() +
    ...
    geom_errorbar(data = flux_year,
                  aes(x = date, ymin = mean_flux - se_flux, 
                      ymax = mean_flux + se_flux, color = Treatment),
                  width = 0.2, alpha = 0.5) +
    # Fertilizer events
  ...
  # Cumulative flux plot with plot-level CI ribbons
  cumulative_plot <- ggplot(cum_flux_year, 
                            aes(x = date, y = cum_flux_mean, color = Treatment)) +
    geom_ribbon(
      aes(ymin = pmax(cum_flux_mean - cum_flux_ci, 0), 
          ymax = cum_flux_mean + cum_flux_ci, 
          fill = Treatment),
      alpha = 0.2, linewidth = 0
    ) +
    geom_line(linewidth = 1.1) +
    geom_point(data = cum_flux_pts, 
               aes(x = date, y = cum_flux_mean, color = Treatment), 
               size = 1.8) +
    scale_color_treatments() +
...
This figure uses a more statistically appropriate uncertainty method for cumulative flux:

1. **Plot-level computation**: Cumulative flux is computed for each individual plot
   by interpolating daily fluxes between measurement dates and summing over time.

2. **Between-plot variability**: At each date, cumulative flux values are summarized
...
3. **Visualization**: The ribbon represents the 95% CI from between-plot variability
   in cumulative flux, which is a more appropriate measure of uncertainty than
