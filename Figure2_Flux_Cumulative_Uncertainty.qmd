...
## Dual-Axis Version (plot-level CI styling)

```{r figure-2-dual-axis-plotlevel}
#| label: fig-n2o-flux-dual-axis-plotlevel
#| fig-cap: "Daily and cumulative N2O flux with precipitation on dual axes (plot-level CI style)"
#| fig-width: 10
#| fig-height: 8

# Global limits for consistent scaling across panels (reuses existing calculations)
...
# Scale precipitation so bars "rain down" from the top of the flux axis
...
create_dual_axis_plot <- function(year_val, is_right_panel = FALSE) {
  ...
  cum_flux_year <- cum_flux_summary %>% filter(year == year_val)

  # Full-year limits to align x-axes
  ...
  daily_plot <- ggplot() +
   ...
   #           linewidth = 1.1, alpha = 0.85) +
    ...
    scale_y_continuous(
      name = if (!is_right_panel) expression("Daily N"[2]*"O Flux (g N ha"^-1*" day"^-1*")") else "",
...
figure2_dual_axis_plotlevel <- (plot_2023 | plot_2024) +
  plot_layout(guides = "collect") &
  theme(legend.position = "bottom",
        legend.justification = "center",
        legend.box.just = "center")

figure2_dual_axis_plotlevel
```
...
ggsave(
  filename = "figures/figure2_flux_precip_cumulative_2023_2024_plotlevelCI_log.png",
...
ggsave(
  ...
  plot     = figure2_dual_axis_plotlevel,
  width    = 174,
  height   = 234,
  units    = "mm",
  dpi      = 600,
...
Get range of peak flux for sorghum and sorghum + rye treatments

```{r}
target_treatments <- c("Sorghum", "Sorghum + Rye")

peak_stats <- flux_data %>%
  mutate(year = year(date)) %>%
  filter(treatment %in% target_treatments) %>%
  filter(year %in% c(2023, 2024)) %>%
  group_by(plot, year) %>%
  summarise(
    max_flux = max(gnha_day_no_negative, na.rm = TRUE), .groups = "drop"
  ) %>%
  group_by(year) %>%
  summarise(
    min_flux = min(max_flux, na.rm = TRUE),
